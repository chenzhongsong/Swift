...
class BookViewController: UIViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    @IBAction func setting(sender: AnyObject) {
        if User.sharedUser.isLogin {
            navigationController?.pushViewController(UIStoryboard.instantiateWithType(.User, identifier: "UserContainerViewController"), animated: true)
        } else {
            navigationController?.pushViewController(UIStoryboard.instantiateWithType(.User, identifier: "LoginViewController"), animated: true)
        }
    }
