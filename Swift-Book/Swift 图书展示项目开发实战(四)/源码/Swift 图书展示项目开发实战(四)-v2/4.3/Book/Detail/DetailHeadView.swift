...
class DetailHeadView: UIView {
    ...
    @IBAction func comment(sender: AnyObject) {
        if User.sharedUser.isLogin {
            let reviewContainerVC = UIStoryboard.instantiateWithType(.User, identifier: "ReviewContainerViewController") as! ReviewContainerViewController
            reviewContainerVC.book = book
            vc.navigationController?.pushViewController(reviewContainerVC, animated: true)
        } else {
    ...
    @IBAction func collect(sender: AnyObject) {
        if User.sharedUser.isLogin {
            let storeContainerVC = UIStoryboard.instantiateWithType(.User, identifier: "StoreContainerViewController") as! StoreContainerViewController
            storeContainerVC.book = book
            vc.navigationController?.pushViewController(storeContainerVC, animated: true)
        } else {
            vc.navigationController?.pushViewController(UIStoryboard(name: "User", bundle: nil).instantiateViewControllerWithIdentifier("LoginViewController"), animated: true)
        }
    }
